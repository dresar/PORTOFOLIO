...
export const api = {
  ...
  content: {
    ...
    settings: {
      get: async (): Promise<any> => {
        const data = await fetchWithLocalFallback<any>('/settings', 'settings_cache', {});
  ...
  socialLinks: {
    getAll: async (): Promise<SocialLink[]> => {
      return fetchWithLocalFallback<SocialLink[]>('/social-links', 'social_links_cache', []);
    },
  ...
  projects: {
    ...
    create: async (data: CreateProjectDTO): Promise<Project> => {
      ...
      // Handle summaries if provided in initial creation (only if data.summaries is a real array)
      if (Array.isArray(data.summaries) && data.summaries.length > 0 && response.data.id) {
        const savedSummaries = response.data.summaries;
        const hasSavedSummaries = savedSummaries && savedSummaries !== '[]';
  ...
  projectCategories: {
    getAll: async (): Promise<ProjectCategory[]> => {
      return fetchWithLocalFallback<ProjectCategory[]>('/projects/categories', 'project_categories_cache', []);
    },
  ...
  skillCategories: {
    getAll: async (): Promise<SkillCategory[]> => {
      return fetchWithLocalFallback<SkillCategory[]>('/skill-categories', 'skill_categories_cache', [
  ...
  education: {
    getAll: async (): Promise<Education[]> => {
      return fetchWithLocalFallback<Education[]>('/education', 'education_cache', []);
    },
  ...
  certificates: {
    getAll: async (): Promise<Certificate[]> => {
      return fetchWithLocalFallback<Certificate[]>('/certificates', 'certificates_cache', []);
    },
  ...
  certificateCategories: {
    getAll: async (): Promise<CertificateCategory[]> => {
      return fetchWithLocalFallback<CertificateCategory[]>('/certificate-categories', 'certificate_categories_cache', []);
    },
