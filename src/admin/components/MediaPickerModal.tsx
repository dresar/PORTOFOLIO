...
export function MediaPickerModal({ isOpen, onClose, onSelect }: MediaPickerModalProps) {
  ...
  const [resourceTypeTab, setResourceTypeTab] = useState<'image' | 'video' | 'raw'>('image');
  ...
  const cloudinaryConfigs = configs.filter((c: any) => c.id !== 9999 && c.id !== 9998);
  ...
  const deleteMutation = useMutation({
    mutationFn: (asset: CloudinaryAsset) =>
      cloudinaryApi.deleteAsset(asset.public_id, resourceTypeTab, asset.provider, asset.sha),
    onSuccess: () => {
      toast({ title: '✓ Dihapus!' });
      qc.invalidateQueries({ queryKey: ['cloudinary-assets'] });
  ...
  const handleOpenAssetPreview = (asset: CloudinaryAsset) => {
    const isPdf = asset.format === 'pdf' || /\.pdf($|\?)/i.test(asset.secure_url || asset.url);
    if (isPdf) {
            ...
            <div className="flex items-center justify-between p-4 border-b border-border/50 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{filtered.length} aset ditemukan</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Button size="sm" onClick={() => setShowUpload(true)} className="gap-1.5 h-8 text-xs rounded-lg">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-1 bg-muted/60 p-1 rounded-lg border border-border/50">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="h-7 text-xs font-medium gap-1 px-2.5 rounded-md"
                    onClick={() => setProviderFilter('r2')}
                  >
                ...
                <div className="flex items-center gap-1 bg-muted/60 p-1 rounded-lg">
                  <Button
                    ...
                    size="sm"
                    className="h-7 text-xs gap-1.5 font-medium rounded-md"
                  ...
                  {cloudinaryConfigs.map((cfg: any) => (
                    <button
                      ...
                      className={cn(
                        ...
                        cfg.is_active
                          ? 'bg-sky-500/15 border-sky-500/30 text-sky-700 dark:text-sky-400 shadow-xs'
                          : 'border-border/50 hover:bg-muted text-muted-foreground'
                      )}
                    >
                      <Cloud className="size-3" />
                      {cfg.label || cfg.cloud_name}
                      {cfg.is_active && <Check className="size-3 text-sky-500" />}
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-48 gap-3 text-center border border-dashed rounded-2xl">
                  ...
                  {!search && (
                    <Button size="sm" onClick={() => setShowUpload(true)} className="rounded-lg">
                      Upload
                    </Button>
                  )}
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {asset.resource_type === 'video' ? (
                            <div className="size-full bg-black flex items-center justify-center relative">
                              ...
                              <div className="absolute inset-0 m-auto size-8 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white pointer-events-none z-10">
                          ...
                          <div className="absolute top-1.5 left-1.5 z-10">
                            {asset.provider === 'r2' ? (
                              <span className="bg-orange-600/90 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-md flex items-center gap-1 shadow backdrop-blur-sm">
                                <CloudUpload className="size-2" /> Cloudflare R2
                              </span>
                              ...
                              <span className="bg-emerald-600/90 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-md flex items-center gap-1 shadow backdrop-blur-sm">
                                <Server className="size-2" /> GitHub CDN
                              </span>
                            ) : (
                              <span className="bg-sky-600/90 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-md flex items-center gap-1 shadow backdrop-blur-sm">
                                <Cloud className="size-2" /> Cloudinary
                              </span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-1.5 p-2">
                          ...
                          >
                            {copiedId === asset.public_id ? <Check className="size-3 text-green-500" /> : <Copy className="size-3" />}
                            Salin
                          </Button>
                          {confirmDelete === asset.public_id ? (
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              className="w-full h-7 text-xs gap-1 rounded-lg hover:bg-destructive hover:text-white hover:border-destructive"
                        ...
                        <div className="p-1.5 border-t border-border/40">
                          <p className="text-xs text-muted-foreground truncate font-medium" title={asset.public_id}>
                            {asset.public_id.split('/').pop()}
                          </p>
                          <p className="text-[11px] text-muted-foreground/60">{formatBytes(asset.bytes)}</p>
                        </div>
                      </motion.div>
                    );
                  })}
              ...
              <div className="flex items-center justify-between p-4 border-b border-border/50 bg-muted/20">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    size="sm"
                    variant="secondary"
                    className="h-8 gap-1.5 text-primary text-xs rounded-lg"
                    onClick={() => handleCopy(previewImage.secure_url, previewImage.public_id)}
                  >
                    {copiedId === previewImage.public_id ? <Check className="size-3.5 text-green-500" /> : <Copy className="size-3.5" />}
                    <span>Salin</span>
                  </Button>
                  <Button
                    size="icon"
                ...
                <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="capitalize">
                      {previewImage.provider === 'r2'
                        ? 'Cloudflare R2 (r2.ekasyarif.my.id)'
                        : previewImage.provider === 'github'
                        ? 'GitHub CDN (jsDelivr Edge)'
                        : `Cloudinary (${previewImage._account || ''})`}
                    </span>
                  </div>
