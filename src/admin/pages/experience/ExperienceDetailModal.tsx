        ...
        <ScrollArea className="flex-1 w-full p-6 pt-2">
          <div className="space-y-6 pb-6">
            <div className="flex flex-wrap gap-2">
              ...
              {experience.isCurrent && <Badge className="text-sm bg-green-500 hover:bg-green-600">Aktif</Badge>}
            </div>
            ...
            {experience.description && (
              <div className="space-y-2">
                <h3 className="font-semibold text-lg flex items-center gap-2">
                  <Briefcase className="h-5 w-5" /> Deskripsi
                </h3>
                <div 
                  className="html-theme-responsive leading-relaxed prose dark:prose-invert max-w-none [&>p]:mb-2 [&>ul]:list-disc [&>ul]:pl-4 [&>ol]:list-decimal [&>ol]:pl-4"
                  dangerouslySetInnerHTML={{ __html: sanitizeHtmlContent(experience.description) }}
                />
              </div>
            )}
                      ...
                      <div key={idx} className="aspect-video rounded-lg overflow-hidden border bg-muted group relative">
                        {isVideo ? (
                          ...
                          <>
                            ...
                            >
                              <span className="text-white text-xs bg-black/50 px-2 py-1 rounded">Buka</span>
                            </a>
                          </>
                        )}
                      </div>
