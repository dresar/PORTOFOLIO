      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="description">Deskripsi Pekerjaan (HTML & Rich Text)</Label>
              <RichHtmlEditor 
                value={form.watch('description') || ''}
                onChange={(content) => form.setValue('description', content, { shouldDirty: true })}
                placeholder="Jelaskan tanggung jawab, pencapaian, atau tuliskan format HTML..."
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                      <Input id="image" {...form.register('image')} placeholder="https://cdn.example.com/logo.png" />
                      {form.watch('image') && (
                          <div className="h-10 w-10 relative rounded overflow-hidden border shrink-0">
                              <img src={form.watch('image')} alt="Preview Logo" className="h-full w-full object-contain p-1" />
                          </div>
                      )}
                  </div>
                  <p className="text-xs text-muted-foreground">URL gambar logo perusahaan (Direct Link CDN).</p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="coverImage">URL Banner / Cover Image (CDN)</Label>
                  <div className="flex gap-2">
                      <Input id="coverImage" {...form.register('coverImage')} placeholder="https://cdn.example.com/banner.jpg" />
                      {form.watch('coverImage') && (
                          <div className="h-10 w-16 relative rounded overflow-hidden border shrink-0">
                              <img src={form.watch('coverImage')} alt="Preview Cover" className="h-full w-full object-cover" />
                          </div>
                      )}
                  </div>
                  <p className="text-xs text-muted-foreground">URL gambar banner header modal detail (Direct Link CDN).</p>
                </div>
              </div>
