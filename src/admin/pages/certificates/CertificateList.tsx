...
export default function CertificateList() {
  ...
  // Filtering & Sorting Logic (default ID asc)
  const filteredCertificates = useMemo(() => {
    let list = certificates || [];
    if (selectedCategory !== "all") {
      list = list.filter((c: any) => c.categoryId?.toString() === selectedCategory);
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          {selectedIds.length > 0 && (
            <Button variant="destructive" size="sm" onClick={handleBulkDelete} disabled={isDeleting} className="h-9">
          ...
          <Button variant="outline" size="sm" onClick={toggleSelectAll} className="h-9">
            ...
            {filteredCertificates.length > 0 && selectedIds.length === filteredCertificates.length ? 'Batal' : 'Pilih Semua'}
          </Button>
          ...
          <Button variant="outline" size="icon" className="h-9 w-9" onClick={() => queryClient.invalidateQueries({ queryKey: ['certificates'] })} disabled={isFetching}>
            <RefreshCw className={`h-4 w-4 ${isFetching ? 'animate-spin' : ''}`} />
          </Button>

          <Button size="sm" onClick={() => navigate('/admin/certificates/new')} className="h-9">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {currentItems.map((cert) => {
          const categoryObj = categories.find((c: any) => Number(c.id) === Number(cert.categoryId));
          return (
                        ...
                        <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
                          ...
                          {categoryObj && (
                            <Badge variant="secondary" className="text-[10px] px-1.5 py-0">
                              Kat ID: {categoryObj.id} ({categoryObj.name})
                            </Badge>
                          )}
                        </div>
