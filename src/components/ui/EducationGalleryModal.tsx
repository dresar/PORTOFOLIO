  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={(open) => !open && closeModal()}>
      <DialogContent className="max-w-4xl w-full p-0 overflow-hidden bg-card/95 backdrop-blur-md border-border/50">
        <DialogTitle className="sr-only">Galeri {educationData.institution}</DialogTitle>
        <DialogDescription className="sr-only">Dokumentasi galeri riwayat pendidikan</DialogDescription>
                ...
                <div className="w-full h-full flex items-center justify-center text-muted-foreground relative">
                    {educationGallery[currentImageIndex] ? (
                        <img 
                            ...
                            onClick={() => {
                              const urls = educationGallery.map((g: any) => typeof g === 'string' ? g : g.url);
                              const currentSrc = typeof educationGallery[currentImageIndex] === 'string' ? educationGallery[currentImageIndex] : educationGallery[currentImageIndex].url;
                              openImagePreviewModal(currentSrc, `${educationData.institution} - Galeri`, urls, currentImageIndex);
                            }}
                        />
                    ) : (
