      ...
      <div className="container mx-auto px-4 max-w-6xl pt-16 pb-12">
        <div className="mb-14 p-7 sm:p-9 rounded-2xl bg-gradient-to-br from-card via-muted/20 to-card border border-border/80 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 hover:border-primary/40 transition-colors relative overflow-hidden">
          ...
          <div className="space-y-2 text-center md:text-left">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground dark:text-zinc-400 max-w-xl leading-relaxed">
              {isId 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-10 pb-12 border-b border-border/60">
          <div className="lg:col-span-4 space-y-4">
            ...
            <div className="pt-1">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/40 dark:border-emerald-500/30 dark:text-emerald-400 text-xs font-semibold">
              ...
              {navLinks.map((item) => (
                <li key={item.label}>
                  {item.href.startsWith('#') ? (
                    <a 
                      href={item.href} 
                      className="text-muted-foreground hover:text-primary dark:text-zinc-400 transition-colors block py-0.5 font-medium"
                    >
                      {item.label}
                    </a>
                  ) : (
          ...
          <div className="lg:col-span-3 space-y-3">
            ...
            <ul className="space-y-2 text-xs text-muted-foreground dark:text-zinc-300">
              {focusAreas.map((area) => (
                ...
                <li key={res.label}>
                  {res.isExternal ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-muted-foreground hover:text-primary dark:text-zinc-400 transition-colors flex items-center justify-between py-0.5 font-medium group"
                    >
                      <span className="truncate">{res.label}</span>
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-muted text-muted-foreground group-hover:text-primary group-hover:bg-primary/10 dark:text-zinc-300 transition-colors shrink-0 border border-border/50">
                        {res.badge}
                      </span>
                    </a>
                  ) : (
          ...
          <div className="flex items-center gap-1.5">
            {normalizedSocialLinks.map((link) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="h-8 w-8 rounded-lg bg-background border border-border/60 hover:border-primary hover:text-primary flex items-center justify-center transition-all text-muted-foreground hover:text-foreground dark:text-zinc-400 dark:hover:text-white active:scale-[0.98]"
        ...
        <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-muted-foreground dark:text-zinc-400">
          ...
          <div className="flex items-center gap-4">
            <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground dark:text-zinc-300">
              {t('footer.made_with')}{' '}
